...
# Load deep learning model (VGG19)
model_code='VGG19'
...
def get_prediction(file, entrytype='file'):
...
# -------------------------------------------------------------------
# 1 bis/ Status authaurized
# -------------------------------------------------------------------

@api.get('/bloodyspy/authorization', name='Test authorizations of the API')
def get_status_auth(username: str = Depends(get_current_user)):
    """Returns a status=1 if the API is working and the user is authorized.
    """
    return {
            "status": 1,
            "message": "Hello {} from a private endpoint. You can see this message because we're authorizing your connection.".format(username)
    }
